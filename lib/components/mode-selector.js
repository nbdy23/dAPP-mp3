document.addEventListener('DOMContentLoaded', () => {
    ...
    function showStrudelTerminal() {
        ...
        if (!terminal) {
            ...
            const patternLib = createPatternLibrary();
            
            // On desktop, append inside terminal. On mobile, append to body.
            const isDesktop = window.innerWidth >= 769;
            if (isDesktop) {
                terminal.appendChild(patternLib);
                console.log('📍 Pattern library: INSIDE terminal (desktop)');
            } else {
                document.body.appendChild(patternLib);
                console.log('📍 Pattern library: BODY overlay (mobile)');
            }
        }
